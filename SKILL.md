...
- Same-thread `Task`-tool subagents (no separate process, no resume, no ledger) — see `skill-of/agent-dispatch`'s sibling skill, `SKILL-OF/efficient-subagent-use`, for that case. The two overlap only at the edges: its "give the rubric's context inline" rule and this skill's prompt-hygiene section are both about what a cold-started agent needs told explicitly, argued from different angles (cost/scope there, evidence-contamination here).
